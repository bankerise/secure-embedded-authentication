...
/// Hooking an unvalidated heuristic directly into `SEAAuthViewController`'s
/// capture path risks silently rerouting legitimate cancellations/errors
/// into an unwanted `ASWebAuthenticationSession` restart, which is worse
/// than doing nothing.
///
/// What *is* implemented and tested here is the pure decision function
/// itself, `indicatesWebauthnUnavailable(_:)`, plus this documented
/// integration seam:
///
/// - Call it from wherever the host SDK / a later SEACore change decides to
///   inspect an `onCaptured` delivery for a Keycloak error redirect (the
///   params reaching `onCaptured` are exactly `SEACallbackParams` — see
///   contract §3.4/§6.3, error-shaped callbacks are delivered through
///   `onCaptured`, never `onError`).
/// - If it returns `true`, the caller is expected to emit
///   `AUTH_WEBAUTHN_FALLBACK { reason: "runtime" }` (via
///   `SEATelemetryEventName.webauthnFallback`) and restart the attempt
...
public enum SEAWebAuthnLiveFailure {
    ...
    static let webauthnErrorSignals: Set<String> = [
        "webauthn_error",
        "not_allowed_error",
        "not_supported_error",
        "security_error",
    ...
    public static func indicatesWebauthnUnavailable(_ params: SEACallbackParams) -> Bool {
        guard let error = params.error?.lowercased() else { return false }
